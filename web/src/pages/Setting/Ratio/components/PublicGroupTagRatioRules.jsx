...
function nestRules(rules) {
  const result = {};
  rules.forEach(({ publicGroup, channelTag, ratio }) => {
    if (!publicGroup || !channelTag) return;
    if (!result[publicGroup]) result[publicGroup] = {};
    result[publicGroup][channelTag] = ratio;
  });
  return result;
}

export function serializePublicGroupTagRatio(rules) {
          ...
          {items.map((rule) => (
            ...
            >
              ...
              <InputNumber
                ...
                value={rule.ratio}
                style={{ width: 100 }}
                onChange={(value) => onUpdate(rule._id, 'ratio', value ?? 0)}
              />
              <Popconfirm
                title={t('确认删除该规则？')}
...
export default function PublicGroupTagRatioRules({
  value,
  groupNames = [],
  onChange,
}) {
  ...
  const emitChange = useCallback(
    (newRules) => {
      setRules(newRules);
      onChange?.(serializePublicGroupTagRatio(newRules));
    },
    [onChange],
  );
  ...
  const addRuleToGroup = useCallback(
    (publicGroup) => {
      emitChange([
        ...rules,
        { _id: uid(), publicGroup, channelTag: '', ratio: 1 },
      ]);
    },
    [rules, emitChange],
  );
